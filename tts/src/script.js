...
const renderKeyframes = (
  ...
  if (isWhispering) {
    keyframes.forEach((keyframe) => {
      Object.assign(keyframe, deconstructVoiceness(0.7));
    });
  }
...
const createResultContainer = () => {
  ...
  const setAlternative = (_alternative) => {
    ...
    [...alternative].forEach((phoneme, index) => {
      ...
      let holdTime = 0;
      let intensity = 1;
      if (phonemes[phoneme]?.intensity !== undefined) {
  intensity = phonemes[phoneme].intensity;
}
...
if ("frequency" in phonemeInfo && !isSubPhoneme) {
  keyframe.frequency = phonemeInfo.frequency;
}
